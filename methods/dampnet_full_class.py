...
class DampNet(MetaTemplate):
  ...
  def set_forward(self,x,is_feature=False, domain_shift = False):
    ...
    if domain_shift == False:
      if is_feature:
        ...
        #print(x2[:,:self.n_support,:].shape)
        #print(x2.shape)
        x_mean_class = torch.mean(x2[:,:self.n_support,:], axis = 1).detach()
        #print(x_mean_class.shape)
        #x_mean_class = x_mean_class - torch.mean(x_mean_class)
        #x_mean_class = x_mean_class / x_mean_class.std().detach()

        x_std = x_mean_class.std(axis = 0).detach()
        ##x_std = x_std - torch.mean(x_std)
        ## x_std = x_std / x_std.std().detach()
        #print(x_std.shape)
        #a = lassn
      ...
      if self.final_meta_prototypes_initialized == False:
        ...
        perc_zeros = (0.9 - 0.1) * np.random.random_sample() + 0.1
        a2 = 1.5
        b2 = 5
      ...
      if is_feature:
        ...
        meta_prototype_mean = self.final_meta_prototype
        meta_prototype_std = self.final_meta_prototype_std
        x_mean = torch.mean(x2[:,:self.n_support,:], axis = (0,1)).detach().cuda()
        x_mean_class = torch.mean(x2[:,:self.n_support,:], axis = 1).detach().cuda()
        x_std = x_mean_class.std(axis = 0).detach()
        
        W_out_m = self.W_R(meta_prototype_mean.cuda(), x_mean).cuda()
        V_out_m = self.V_R(torch.cat((meta_prototype_mean.cuda(), x_mean.cuda())))
        NTN_out = W_out_m + V_out_m
  ...
  def train_loop_full(self, epoch, train_loader, optimizer, final_epoch):
        ...
        for i, (x,_ ) in enumerate(train_loader):
            ...
            all_feats[i] = feats
        #if epoch % 5 == 0:
        self.final_all_feats[(epoch % 5)] = all_feats.cuda()
        if epoch >= start:
  ...
  def set_forward_adaptation_full(self, x, is_feature = True): #further adaptation, default is fixing feature and train a new softmax clasifier
        ...
        x2 = x.view(self.n_way, -1, x.size(1))
        ### LOAD PROTOTYPES
        meta_prototype_mean = self.final_meta_prototype.cuda()
        meta_prototype_std = self.final_meta_prototype_std.cuda()
        x_mean = torch.mean(x2[:,:self.n_support,:], axis = (0,1)).detach()
        x_mean_class = torch.mean(x2[:,:self.n_support,:], axis = 1).detach()
        x_std = x_mean_class.std(axis = 0).detach()
        
        W_out_m = self.W_R(meta_prototype_mean.cuda(), x_mean.cuda()).cuda()
        ...
        recovered_x = torch.mul(original_x.detach(), mult_) + add_ ### use back normal x
        recovered_x = recovered_x.view(self.n_way * (self.n_support + self.n_query), -1)
       
        r_z = self.fc(recovered_x)
        r_z = r_z.view(self.n_way ,(self.n_support + self.n_query),-1)
        z_support, z_query  = self.parse_feature(r_z,is_feature)
        ...
        if linear:
          ...
          linear_clf = nn.Linear(self.gnn_dim, self.n_way)
          linear_clf = linear_clf.cuda()

          set_optimizer = torch.optim.SGD(linear_clf.parameters(), lr = 0.01, momentum=0.9, dampening=0.9, weight_decay=0.001)
          #set_optimizer2 = torch.optim.SGD(fc_new.parameters(), lr = 0.01, momentum=0.9, dampening=0.9, weight_decay=0.001)
